...
def create_agents_and_tasks(pdf_tool):
    ...
    retriever_agent = Agent(
        ...
        goal=(
            "Retrieve the most relevant information following these strict steps:\n"
        "1. ALWAYS use the PDF search tool first with this exact format:\n"
        "   DocumentSearchTool(query='your search text here')\n\n"
        "2. ONLY if you cannot find relevant information in the PDF, "
        "use the web search tool with this exact format:\n"
        "   Search the internet(search_query='your search text here')\n\n"
        "Remember: Never pass dictionaries or objects, only simple text strings "
        "as shown in the examples above."
        ),
        backstory=(
            "You're a meticulous analyst with a keen eye for detail, trained to always "
        "prioritize searching in local documents before resorting to web searches. "
        "You're known for your systematic approach to information retrieval, "
        "always checking the PDF thoroughly before looking elsewhere."
        ),
        verbose=True,
